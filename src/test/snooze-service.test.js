...
await import('../app-service/snooze-handler.js')
...
test('onInit создаёт snooze alarm и записывает лог', () => {
  serviceOpts.onInit(JSON.stringify({ intakeId: 'i1', delayMinutes: 30 }))
...
test('onInit игнорирует отсутствующий intake', () => {
  serviceOpts.onInit(JSON.stringify({ intakeId: 'missing', delayMinutes: 30 }))
  const logs = storage.__stores().get('aibolit-data.json').get('takeLogs')
